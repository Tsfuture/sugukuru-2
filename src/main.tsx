...
if (!envCheck.valid) {
  ...
  root.innerHTML = `
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 100vh; font-family: system-ui, sans-serif; padding: 20px; text-align: center; background: #fef2f2;">
      <div style="background: white; padding: 32px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); max-width: 500px;">
        <h1 style="color: #dc2626; margin-bottom: 16px; font-size: 24px;">⚠️ 設定不足</h1>
        ...
        <div style="background: #f3f4f6; padding: 16px; border-radius: 8px; text-align: left;">
          <p style="color: #374151; font-size: 14px; margin-bottom: 8px; font-weight: 600;">Netlify での設定手順:</p>
          <ol style="color: #6b7280; font-size: 13px; padding-left: 20px; margin: 0;">
            <li style="margin: 4px 0;">Netlify Dashboard → Site settings → Environment variables</li>
            <li style="margin: 4px 0;">上記の変数を追加</li>
            <li style="margin: 4px 0;">デプロイを再実行</li>
          </ol>
        </div>
      </div>
    </div>
  ...
  console.error("[supabase] Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in Netlify Environment Variables");
} else {
